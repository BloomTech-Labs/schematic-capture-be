...
router.post("/login", (req, res) => {
	if (!req.body) {
		res.status(400).json({
			message:
				"Missing post data. Ensure you sent the user's login information.",
		});
	} else if (!req.body.username || !req.body.password) {
		res.status(400).json({
			message:
				"Incomplete user data. Please include the user's username and password",
		});
	}
	const loginInfo = {
	...
	axios
		...
		.then((response) => {
			...
			Users.findBy({ email: response.data._embedded.user.profile.login }).then(
				(user) => {
		...
		.catch((err) => {
			res.status(500).json({
				error: err,
				message: "Login with Okta failed.",
				step: "api/auth/login",
			});
		});
});
	...
	(req, res) => {
		...
		axios
			...
			.then((response) => {
				...
				Users.update({ id: req.token.id }, { question: newQuestion })
					.then(() => {
					...
					.catch((err) => {
						res.status(400).json({
							error: err,
							message:
								"Failed to change security question in Schematic Capture database.",
			...
			.catch((err) => {
				...
				res.status(500).json({
					error: err,
					message:
						"Failed to log user in after changing password and security question.",
	...
	Users.getQuestion(req.params.id)
		.then((question) => {
			if (question.question === null) {
				//No question found
				res.status(400).json({
					errorMessage: "No security question was found for the user.",
					step: "api/auth/securityquestion/:id",
				});
			} else {
		...
		.catch((err) => {
			res.status(500).json({
				error: err,
				message: "Couldn't get security question",
				step: "api/auth/securityquestion/:id",
			});
		});
});
...
router.post("/forgotpassword", getIdByEmail, (req, res) => {
	...
	axios
		...
		.catch((err) => {
			res.status(500).json({
				error: err,
				message: "Couldn't reset the password with Okta.",
				step: "api/auth/forgotpassword",
			});
		});
});
...
//TESTED
router.get("/questions", validateIdToken, superRoleIdAuth, (req, res) => {
	//this url will be different
	axios
		.get(
			"https://dev-833124.okta.com/api/v1/users/00u4syc0frXBxcDtF4x6/factors/questions"
		)
		.then((response) => {
			res.status(200).json(response.data);
		})
		.catch((err) => {
			res.status(500).json({
				error: err,
				message: "Couldn't get recovery questions from Okta.",
				step: "api/auth/questions",
			});
		});
});
